...
ALTER TABLE etags ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
...
CREATE VIEW collective_demands AS
 SELECT demands.id,
    demands.seeker_id,
    demands.created_at,
    bodies.build AS body_build,
    bodies.skin,
    bodies.weight,
    bodies.height,
    faces.acne,
    faces.beard,
    faces.complexion AS face_complexion,
    faces.eyebrow,
    faces.freckles AS face_freckles,
    faces.hair,
    faces.left_eye,
    faces.right_eye,
    faces.shape AS face_shape,
    faces.teeth,
...
CREATE UNIQUE INDEX etags_entity_uindex ON etags USING btree (lower(entity));
