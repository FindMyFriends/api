...
COPY bodies (id, build, skin, weight, height) FROM stdin;
\.
...
COPY demands (id, seeker_id, description_id, created_at) FROM stdin;
\.
...
COPY descriptions (id, general_id, body_id, face_id) FROM stdin;
\.
...
COPY faces (id, teeth, freckles, complexion, beard, acne, shape, hair, eyebrow, left_eye, right_eye) FROM stdin;
\.
...
COPY general (id, gender, race, birth_year, firstname, lastname) FROM stdin;
\.
...
COPY seekers (id, email, password, description_id) FROM stdin;
\.
...
SELECT pg_catalog.setval('bodies_id_seq', 1, false);
...
SELECT pg_catalog.setval('demands_id_seq', 1, false);
...
SELECT pg_catalog.setval('descriptions_id_seq', 1, false);
...
SELECT pg_catalog.setval('general_id_seq', 1, false);


--
-- Name: seekers_id_seq; Type: SEQUENCE SET; Schema: public; Owner: postgres
--
...
--
-- Name: seekers seekers_descriptions_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY seekers
    ADD CONSTRAINT seekers_descriptions_id_fk FOREIGN KEY (description_id) REFERENCES descriptions(id) ON DELETE CASCADE;
