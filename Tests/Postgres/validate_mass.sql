...
BEGIN
  RETURN message FROM assert.throws(
    FORMAT('SELECT validate_mass(ROW(%L, %L))', NULL, 'kg'),
...
BEGIN
  RETURN message FROM assert.throws(
    FORMAT('SELECT validate_mass(ROW(%L, %L))', 10, NULL),
...
BEGIN
  RETURN message FROM assert.is_true((SELECT validate_mass(ROW(NULL, NULL))));
END
$$
LANGUAGE plpgsql;
...
BEGIN
  RETURN message FROM assert.is_true((SELECT validate_mass(ROW(10, 'kg'))));
END
$$
LANGUAGE plpgsql;