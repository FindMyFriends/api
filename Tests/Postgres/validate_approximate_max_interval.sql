CREATE FUNCTION unit_tests.throwing_on_overstepped_max() RETURNS test_result AS $BODY$
BEGIN
...
CREATE FUNCTION unit_tests.passing_on_approximation_in_range() RETURNS test_result AS $$
BEGIN
...
CREATE FUNCTION unit_tests.passing_on_approximation_same_as_max() RETURNS test_result AS $$
BEGIN
  RETURN message FROM assert.is_true(
